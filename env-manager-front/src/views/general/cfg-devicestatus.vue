<template>
    <div class="cfgdevicestatus">
    <div class="cfgdevicestatus__filter-container p-filter-container">
        <span class="p-table-name">设备状态列表</span>
    ...
    <section class="cfgdevicestatus__table">
        <el-table element-loading-text="拼命加载中" empty-text="暂无数据"
        :data="list" stripe
        border fit highlight-current-row>
        <el-table-column
        label="设备状态编号" align="center">
        <template slot-scope="scope">
            <span>{{ scope.row.deviceStatus }}</span>
        </template>
        </el-table-column>
        <el-table-column
        label="设备状态名称" align="center">
        <template slot-scope="scope">
            <span>{{ scope.row.name }}</span>
        </template>
        </el-table-column>
        <el-table-column
        label="设备状态英文名称" align="center">
        <template slot-scope="scope">
    ...
    <section class="layer__dialog">
      ...
      <!--  新增 记录 -->
    <section class="layer__dialog">
        <el-dialog title="添加设备状态信息" :visible.sync="isShowAddDialog">
            <el-form ref="addForm" label-position="left"   label-width="200px"  :model="addForm" :rules="rules">
                <el-form-item label="设备状态编号" prop="deviceStatus" >
                     <el-input-number v-model="addForm.deviceStatus" :min="1" :max="255" placeholder="请输入设备状态编号"></el-input-number>
                </el-form-item>
                <el-form-item label="设备状态名称" prop="name" >
                    <el-input v-model="addForm.name" placeholder="请输入设备状态名称"></el-input>
                </el-form-item>
                <el-form-item label="设备状态英文名称" prop="nameEn" >
                    <el-input v-model="addForm.nameEn" placeholder="请输入设备状态英文名称"></el-input>
                </el-form-item>
                <el-row type="flex" >
                    <el-col class="p-oneline">
...
export default {
  ...
  data() {
    return {
      ...
      modifyFormRules: {
        devicestatus: [{ required: true, message: "设备状态编号1-255", trigger: "blur", type: 'number' }],
        name: [{ required: true, message: "设备状态名称", trigger: "blur" }],
        nameEn: [{ required: true, message: "设备状态英文名称", trigger: "blur" }]
      },
      rules: {
        devicestatus: [{ required: true, message: "设备状态编号1-255", trigger: "blur", type: 'number' }],
        name: [
          { required: true, message: "设备状态名称", trigger: "blur" },
  ...
  methods: {
    ...
    handleDelete(index, row) {
      const me = this
      deleteItem("/cfgdevicestatus/", { deviceStatus: row.deviceStatus }).then(response => {
        const { success, data, msg } = response.data
        if (success) {
    ...
    confirmModify() {
      ...
      me.$refs.modifyForm.validate(valid => {
        if (valid) {
          modifyItem('/cfgdevicestatus/', me.modifyForm)
            .then(response => {
              const { success, data, msg } = response.data
    ...
    confirmAdd() {
      ...
      me.$refs.addForm.validate(valid => {
        if (valid) {
          addItem('/cfgdevicestatus/', me.addForm)
            .then(response => {
              const { success, data, msg } = response.data
