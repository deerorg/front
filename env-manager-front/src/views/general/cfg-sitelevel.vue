<template>
    <div class="cfgsitelevel">
    <div class="cfgsitelevel__filter-container p-filter-container">
    ...
    <section class="cfgsitelevel__table">
        <el-table element-loading-text="拼命加载中" empty-text="暂无数据"
        :data="list" stripe
        border fit highlight-current-row>
        <el-table-column label="站点级别编号" align="center">
        <template slot-scope="scope">
            <span>{{ scope.row.siteLevel }}</span>
        </template>
        </el-table-column>
        <el-table-column label="站点级别名称" align="center">
        <template slot-scope="scope">
            <span>{{ scope.row.name }}</span>
        </template>
        </el-table-column>
        <el-table-column label="站点级别英文名称" align="center">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    handleDelete(index, row) {
      const me = this
      deleteItem("/cfgsitelevel/", { siteLevel: row.siteLevel }).then(response => {
        const { success, data, msg } = response.data
        if (success) {
    ...
    confirmModify() {
      ...
      me.$refs.modifyForm.validate(valid => {
        if (valid) {
          modifyItem('/cfgsitelevel/', me.modifyForm)
            .then(response => {
              const { success, data, msg } = response.data
    ...
    confirmAdd() {
      ...
      me.$refs.addForm.validate(valid => {
        if (valid) {
          addItem('/cfgsitelevel/', me.addForm)
            .then(response => {
              const { success, data, msg } = response.data
