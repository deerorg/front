<template>
    <div class="cfgtelestrategy">
    <div class="cfgtelestrategy__filter-container p-filter-container">
    ...
    <section class="cfgtelestrategy__table">
...
<script>
import { getAll, putItem, addItem, deleteItem, modifyItem, getInfo } from "@/api/advanced"

export default {
  name: "cfgtelestrategy",
  data() {
    return {
      rawList: [],
      list: [],
  ...
  methods: {
    ...
    getList() {
      const me = this
      getInfo(`/cfgtelestrategy/list?queryLike=${me.searchName}&pageNum=${me.pageParmas.pageNum}&pageSize=${me.pageParmas.pageSize}`)
        .then(response => {
          const { success, data, msg } = response.data
          if (success) {
            me.list = data.list || []
