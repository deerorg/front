<template>
    <div class="cfgtelemeter">
    <div class="cfgtelemeter__filter-container p-filter-container">
    ...
    <section class="cfgtelemeter__table">
        <el-table element-loading-text="拼命加载中" empty-text="暂无数据"
        :data="list" stripe
        border fit highlight-current-row>
        <el-table-column fixed  label="站点" align="center">
        ...
        <el-table-column label="基本遥测量类型" align="center">
            <template slot-scope="scope">
        ...
        <el-table-column label="有效性表达式变量串" align="center">
            <template slot-scope="scope">
                <span>{{ scope.row.validMid }}</span>
            </template>
        </el-table-column>
          <el-table-column label="有效性判断表达式" align="center">
            <template slot-scope="scope">
                <span>{{ scope.row.validString }}</span>
            </template>
        </el-table-column>
        ...
        <el-table-column label="当前更新值更新时间" align="center">
            <template slot-scope="scope">
        ...
        <el-dialog title="添加遥测量基本信息" :visible.sync="isShowAddDialog">
            <el-form ref="addForm" label-position="left"  label-width="200px"  :model="addForm" :rules="rules">
                ...
                <el-form-item label="遥测量类型名称" prop="metetypeName" >
                 ...
                 <el-form-item label="遥测量类型英文名称" prop="metetypeNameEn" >
                ...
                <el-form-item label="有效下限" prop="lowEffect" >
                 ...
                 <el-form-item label="精度" prop="accuracy" >
                    <el-input-number v-model="addForm.accuracy" placeholder="精度" :min="0" :max="3"></el-input-number>
                </el-form-item>
                <el-form-item label="单位" prop="unit" >
...
export default {
  ...
  methods: {
    ...
    handleDelete(index, row) {
      const me = this
      deleteItem("/cfgtelemeter/", { siteLevel: row.siteLevel }).then(response => {
        const { success, data, msg } = response.data
        if (success) {
