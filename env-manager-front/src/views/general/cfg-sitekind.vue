<template>
    <div class="cfgsitekind">
    <div class="cfgsitekind__filter-container p-filter-container">
    ...
    <section class="cfgsitekind__table">
...
export default {
  ...
  methods: {
    ...
    handleDelete(index, row) {
      const me = this
      deleteItem("/cfgsitekind/", { siteKind: row.siteKind }).then(response => {
        const { success, data, msg } = response.data
        if (success) {
    ...
    confirmModify() {
      const me = this
      modifyItem('/cfgsitekind/', me.modifyForm)
        .then(response => {
          const { success, data, msg } = response.data
    ...
    confirmAdd() {
      const me = this
      addItem('/cfgsitekind/', me.addForm)
        .then(response => {
          const { success, data, msg } = response.data
