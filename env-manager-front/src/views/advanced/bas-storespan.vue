<template>
    <div class="basstorespan">
    <div class="basstorespan__filter-container p-filter-container">
    ...
    <section class="basstorespan__table">
...
export default {
  ...
  methods: {
    ...
    handleDelete(index, row) {
      const me = this
      deleteItem("/storespan/", { spanType: row.spanType }).then(response => {
        const { success, data, msg } = response.data
        if (success) {
    ...
    confirmModify() {
      ...
      me.$refs.modifyForm.validate(valid => {
        if (valid) {
          modifyItem("/storespan/", me.modifyForm)
            .then(response => {
              const { success, data, msg } = response.data
