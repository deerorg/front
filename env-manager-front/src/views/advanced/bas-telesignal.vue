<template>
    <div class="bastelesignal">
    <div class="bastelesignal__filter-container p-filter-container">
    ...
    <section class="bastelesignal__table">
...
export default {
  ...
  methods: {
    ...
    confirmModify() {
      const me = this
      // PUT /telesignal/{basMeteId}/{deviceType}/{meteType}
      const currentItem = me.modifyForm
      // let pathUrl =  `/telesignal/${currentItem.basMeteId}/${currentItem.deviceType}/${currentItem.meteType}`;
      me.$refs.modifyForm.validate(valid => {
        if (valid) {
          modifyItem('/telesignal/', me.modifyForm).then(response => {
            const { success, data, msg } = response.data
