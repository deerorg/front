<template>
    <div class="bassignalkind">
    <div class="bassignalkind__filter-container p-filter-container">
        <span class="p-table-name">遥信量类型列表</span>
    ...
    <section class="bassignalkind__table">
        <el-table element-loading-text="拼命加载中" empty-text="暂无数据"
        :data="list" stripe
        border fit highlight-current-row>
        <el-table-column
        label="遥信量类型编号" align="center">
        <template slot-scope="scope">
            <span>{{ scope.row.signalKind }}</span>
        </template>
        </el-table-column>
        <el-table-column
        label="遥信量类型名称" align="center">
        <template slot-scope="scope">
            <span>{{ scope.row.name }}</span>
        </template>
        </el-table-column>
        <el-table-column
        label="遥信量类型英文名称" align="center">
        <template slot-scope="scope">
    ...
    <section class="layer__dialog">
        <el-dialog title="修改遥信量类型信息" :visible.sync="isShowEditDialog">
            <el-form ref="modifyForm" label-width="200px" label-position="left"    :model="modifyForm" :rules="modifyFormRules">
                <el-form-item label="遥信量类型编号" prop="signalKind">
                    <el-input v-model="modifyForm.signalKind" readonly disabled></el-input>
                </el-form-item>
                <el-form-item label="遥信量类型名称" prop="name" >
                    <el-input v-model="modifyForm.name"></el-input>
                </el-form-item>
                <el-form-item label="遥信量类型英文名称" prop="nameEn" >
      ...
      <!--  新增 记录 -->
    <section class="layer__dialog">
        <el-dialog title="添加遥信量类型信息" :visible.sync="isShowAddDialog">
            <el-form ref="addForm" label-width="200px" label-position="left"   :model="addForm" :rules="rules">
                <el-form-item label="遥信量类型编号" prop="signalKind" >
                     <el-input-number v-model="addForm.signalKind" :min="1" :max="255" placeholder="请输入遥信量类型编号"></el-input-number>
                </el-form-item>
                <el-form-item label="遥信量类型名称" prop="name" >
                    <el-input v-model="addForm.name" placeholder="请输入遥信量类型名称"></el-input>
                </el-form-item>
                <el-form-item label="遥信量类型英文名称" prop="nameEn" >
                    <el-input v-model="addForm.nameEn" placeholder="请输入遥信量类型英文名称"></el-input>
                </el-form-item>
                <el-row type="flex" >
                    <el-col class="p-oneline">
...
export default {
  ...
  data() {
    return {
      ...
      modifyFormRules: {
        signalKind: [{ required: true, message: "遥信量类型编号1-255", trigger: "blur", type: 'number' }],
        name: [{ required: true, message: "遥信量类型名称", trigger: "blur" }],
        nameEn: [{ required: true, message: "遥信量类型英文名称", trigger: "blur" }]
      },
      rules: {
        signalKind: [{ required: true, message: "遥信量类型编号1-255", trigger: "blur", type: 'number' }],
        name: [
          { required: true, message: "遥信量类型名称", trigger: "blur" },
  ...
  methods: {
    ...
    handleDelete(index, row) {
      const me = this
      deleteItem("/signalkind/", { signalKind: row.signalKind }).then(response => {
        const { success, data, msg } = response.data
        if (success) {
    ...
    confirmModify() {
      ...
      me.$refs.modifyForm.validate(valid => {
        if (valid) {
          modifyItem('/signalkind/', me.modifyForm)
            .then(response => {
              const { success, data, msg } = response.data
