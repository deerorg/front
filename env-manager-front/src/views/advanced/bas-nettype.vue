<template>
    <div class="basnettype">
    <div class="basnettype__filter-container p-filter-container">
        <span class="p-table-name">组网类型列表</span>
  ...
  <section class="p-input-wrapper">
    ...
    <section class="basnettype__table">
        <el-table element-loading-text="拼命加载中" empty-text="暂无数据"
        :data="list" stripe
        border fit highlight-current-row>
        <el-table-column
        label="组网类型编号" align="center">
        <template slot-scope="scope">
            <span>{{ scope.row.netType }}</span>
        </template>
        </el-table-column>
        <el-table-column
        label="组网类型名称" align="center">
        <template slot-scope="scope">
            <span>{{ scope.row.name }}</span>
        </template>
        </el-table-column>
        <el-table-column
        label="组网类型英文名称" align="center">
        <template slot-scope="scope">
    ...
    <section class="layer__dialog">
        <el-dialog title="修改组网类型信息" :visible.sync="isShowEditDialog">
            <el-form ref="modifyForm" label-position="left"   label-width="200px"  :model="modifyForm" :rules="modifyFormRules">
                <el-form-item label="组网类型编号" prop="netType">
                    <el-input v-model="modifyForm.netType" readonly disabled></el-input>
                </el-form-item>
                <el-form-item label="组网类型名称" prop="name" >
                    <el-input v-model="modifyForm.name"></el-input>
                </el-form-item>
                <el-form-item label="组网类型英文名称" prop="nameEn" >
      ...
      <!--  新增 记录 -->
    <section class="layer__dialog">
        <el-dialog title="添加组网类型信息" :visible.sync="isShowAddDialog">
            <el-form ref="addForm" label-position="left"   label-width="200px"  :model="addForm" :rules="rules">
                <el-form-item label="组网类型编号" prop="netType" >
                     <el-input-number v-model="addForm.netType" :min="1" :max="255" placeholder="请输入组网类型编号"></el-input-number>
                </el-form-item>
                <el-form-item label="组网类型名称" prop="name" >
                    <el-input v-model="addForm.name" placeholder="请输入组网类型名称"></el-input>
                </el-form-item>
                <el-form-item label="组网类型英文名称" prop="nameEn" >
                    <el-input v-model="addForm.nameEn" placeholder="请输入组网类型英文名称"></el-input>
                </el-form-item>
                <el-row type="flex" >
                    <el-col class="p-oneline">
...
export default {
  ...
  data() {
    return {
      ...
      modifyFormRules: {
        nettype: [{ required: true, message: "组网类型编号1-255", trigger: "blur", type: 'number' }],
        name: [{ required: true, message: "组网类型名称", trigger: "blur" }],
        nameEn: [{ required: true, message: "组网类型英文名称", trigger: "blur" }]
      },
      rules: {
        nettype: [{ required: true, message: "组网类型编号1-255", trigger: "blur", type: 'number' }],
        name: [
          { required: true, message: "组网类型名称", trigger: "blur" },
  ...
  methods: {
    ...
    handleDelete(index, row) {
      const me = this
      deleteItem("/nettype/", { netType: row.netType }).then(response => {
        const { success, data, msg } = response.data
        if (success) {
    ...
    confirmModify() {
      ...
      me.$refs.modifyForm.validate(valid => {
        if (valid) {
          modifyItem('/nettype/', me.modifyForm)
            .then(response => {
              const { success, data, msg } = response.data
