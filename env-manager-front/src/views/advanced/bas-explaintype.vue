<template>
    <div class="basexplaintype">
    <div class="basexplaintype__filter-container p-filter-container">
        <span class="p-table-name">遥信量解释类型列表</span>
    ...
    <section class="basexplaintype__table">
        ...
        <el-table-column
        label="遥信量解释类型名称" align="center">
        <template slot-scope="scope">
            <span>{{ scope.row.name }}</span>
        </template>
        </el-table-column>
        <el-table-column
        label="遥信量解释类型英文名称" align="center">
        <template slot-scope="scope">
    ...
    <section class="layer__dialog">
        <el-dialog title="修改遥信量解释类型信息" :visible.sync="isShowEditDialog">
            <el-form label-position="left" ref="modifyForm" label-width="200px"  :model="modifyForm" :rules="modifyFormRules">
                <el-form-item label="遥信量解释类型编号" prop="explainType">
                    <el-input v-model="modifyForm.explainType" readonly disabled></el-input>
                </el-form-item>
                <el-form-item label="遥信量解释类型名称" prop="name" >
                    <el-input v-model="modifyForm.name"></el-input>
                </el-form-item>
                <el-form-item label="遥信量解释类型英文名称" prop="nameEn" >
      ...
      <!--  新增 记录 -->
    <section class="layer__dialog">
        <el-dialog title="添加遥信量解释类型信息" :visible.sync="isShowAddDialog">
            <el-form ref="addForm" label-position="left"  label-width="200px"  :model="addForm" :rules="rules">
                <el-form-item label="遥信量解释类型编号" prop="explainType" >
                     <el-input-number v-model="addForm.explainType" placeholder="请输入遥信量解释类型编号"></el-input-number>
                </el-form-item>
                <el-form-item label="遥信量解释类型名称" prop="name" >
                    <el-input v-model="addForm.name" placeholder="请输入遥信量解释类型名称"></el-input>
                </el-form-item>
                <el-form-item label="遥信量解释类型英文名称" prop="nameEn" >
                    <el-input v-model="addForm.nameEn" placeholder="请输入遥信量解释类型英文名称"></el-input>
                </el-form-item>
                <el-row type="flex" >
                    <el-col class="p-oneline">
...
export default {
  ...
  data() {
    return {
      ...
      modifyFormRules: {
        name: [{ required: true, message: "遥信量解释类型名称", trigger: "blur" }],
        nameEn: [{ required: true, message: "遥信量解释类型英文名称", trigger: "blur" }]
      },
      rules: {
        explainType: [{ required: true, message: "遥信量解释类型编号1-65535", trigger: "blur", type: 'number' }],
        name: [
          { required: true, message: "遥信量解释类型名称", trigger: "blur" },
  ...
  methods: {
    ...
    handleDelete(index, row) {
      const me = this
      deleteItem("/explaintype/", { explainType: row.explainType }).then(response => {
        const { success, data, msg } = response.data
        if (success) {
    ...
    confirmModify() {
      ...
      me.$refs.modifyForm.validate(valid => {
        if (valid) {
          modifyItem('/explaintype/', me.modifyForm)
            .then(response => {
              const { success, data, msg } = response.data
    ...
    confirmAdd() {
      ...
      me.$refs.addForm.validate(valid => {
        if (valid) {
          addItem('/explaintype/', me.addForm)
            .then(response => {
              const { success, data, msg } = response.data
