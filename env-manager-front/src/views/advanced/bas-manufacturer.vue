<template>
    <div class="basmanufacturer">
    <div class="basmanufacturer__filter-container p-filter-container">
        <span class="p-table-name">制造商列表</span>
    ...
    <section class="basmanufacturer__table">
        <el-table element-loading-text="拼命加载中" empty-text="暂无数据"
        :data="list" stripe
        border fit highlight-current-row>

        <el-table-column label="制造商编号" align="center">
            <template slot-scope="scope">
                <span>{{ scope.row.manufacturerId }}</span>
            </template>
        </el-table-column>
        <el-table-column label="制造商名称" align="center">
            <template slot-scope="scope">
                <span>{{ scope.row.name }}</span>
            </template>
        </el-table-column>
        <el-table-column label="制造商英文名称" align="center">
        <template slot-scope="scope">
            <span>{{ scope.row.nameEn }}</span>
        </template>
        </el-table-column>
        <el-table-column label="制造商地址" align="center">
            <template slot-scope="scope">
                <span>{{ scope.row.address }}</span>
            </template>
        </el-table-column>
        <el-table-column label="制造商英文地址" align="center">
            <template slot-scope="scope">
    ...
    <section class="layer__dialog">
        <el-dialog title="修改制造商信息" :visible.sync="isShowEditDialog">
            <el-form ref="modifyForm" label-position="left"   label-width="200px"  :model="modifyForm" :rules="modifyFormRules">
                <el-form-item label="制造商编号">
                    <el-input v-model="modifyForm.manufacturerId" readonly disabled></el-input>
                </el-form-item>
                <el-form-item label="制造商名称" prop="name" >
                    <el-input v-model="modifyForm.name"></el-input>
                </el-form-item>
                <el-form-item label="制造商英文名称" prop="nameEn" >
                    <el-input v-model="modifyForm.nameEn"></el-input>
                </el-form-item>
                 <el-form-item label="制造商地址">
                    <el-input v-model="modifyForm.address"></el-input>
                </el-form-item>
                <el-form-item label="制造商英文地址" >
                    <el-input v-model="modifyForm.addressEn"></el-input>
                </el-form-item>
      ...
      <!--  新增 记录 -->
    <section class="layer__dialog">
        <el-dialog title="添加制造商信息" :visible.sync="isShowAddDialog">
...
export default {
  ...
  data() {
    return {
      ...
      modifyFormRules: {
        name: [{ required: true, message: "制造商名称", trigger: "blur" }],
        nameEn: [{ required: true, message: "制造商英文名称", trigger: "blur" }]
      },
      rules: {
        name: [
          { required: true, message: "制造商名称", trigger: "blur" },
