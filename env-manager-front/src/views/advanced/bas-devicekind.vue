<template>
    <div class="basdevicekind">
    <div class="basdevicekind__filter-container p-filter-container">
        <span class="p-table-name">设备种类列表</span>
    ...
    <section class="basdevicekind__table">
        <el-table element-loading-text="拼命加载中" empty-text="暂无数据"
        :data="list" stripe
        border fit highlight-current-row>
        <el-table-column
        label="设备种类编号" align="center">
        <template slot-scope="scope">
            <span>{{ scope.row.deviceKind }}</span>
        </template>
        </el-table-column>
        <el-table-column
        label="设备种类名称" align="center">
        <template slot-scope="scope">
            <span>{{ scope.row.kindName }}</span>
        </template>
        </el-table-column>
        <el-table-column
        label="设备种类英文名称" align="center">
        <template slot-scope="scope">
    ...
    <section class="layer__dialog">
        <el-dialog title="修改设备种类信息" :visible.sync="isShowEditDialog">
            <el-form ref="modifyForm" label-position="left"   label-width="200px"  :model="modifyForm" :rules="modifyFormRules">
                <el-form-item label="设备种类编号" prop="devicekind">
                    <el-input v-model="modifyForm.deviceKind" readonly disabled></el-input>
                </el-form-item>
                <el-form-item label="设备种类名称" prop="name" >
                    <el-input v-model="modifyForm.kindName"></el-input>
                </el-form-item>
                <el-form-item label="设备种类英文名称" prop="nameEn" >
      ...
      <!--  新增 记录 -->
    <section class="layer__dialog">
        <el-dialog title="添加设备种类信息" :visible.sync="isShowAddDialog">
            <el-form ref="addForm" label-position="left"  label-width="200px"  :model="addForm" :rules="rules">
                <el-form-item label="设备种类编号" prop="deviceKind" >
                     <el-input-number v-model="addForm.deviceKind" :min="1" :max="255" placeholder="请输入设备种类编号"></el-input-number>
                </el-form-item>
                <el-form-item label="设备种类名称" prop="kindName" >
                    <el-input v-model="addForm.kindName" placeholder="请输入设备种类名称"></el-input>
                </el-form-item>
                <el-form-item label="设备种类英文名称" prop="kindNameEn" >
                    <el-input v-model="addForm.kindNameEn" placeholder="请输入设备种类英文名称"></el-input>
                </el-form-item>
                <el-row type="flex" >
                    <el-col class="p-oneline">
...
export default {
  ...
  data() {
    return {
      ...
      modifyFormRules: {
        deviceKind: [{ required: true, message: "设备种类编号1-255", trigger: "blur", type: 'number' }],
        kindName: [{ required: true, message: "设备种类名称", trigger: "blur" }],
  ...
  methods: {
    ...
    handleDelete(index, row) {
      const me = this
      deleteItem("/devicekind/" + row.devicekind).then(response => {
        const { success, data, msg } = response.data
        if (success) {
    ...
    confirmModify() {
      ...
      me.$refs.modifyForm.validate(valid => {
        if (valid) {
          modifyItem('/devicekind/', me.modifyForm)
            .then(response => {
              const { success, data, msg } = response.data
